...
public class RigitzOld {
    ...
    public RigitzOld(int states, int start, int[] ends, String[][][] changes) {
        ...
        if(dp){
            dpArray = new String[states+1][states][states];
        }
    }
    ...
    public void rigitz(){
        ...
        System.out.println(union(solutions));
    }
    private String rigitz(int k, int i, int j){
        String newRigitz = "";
        if(k == 0 && i == j){
            List<String> withE = new ArrayList<>(changes[i-1][i-1]);
            withE.add(epsilon);
            newRigitz = union(withE);

        }
        else if(k == 0) {
            newRigitz = union(changes[i-1][j-1]);
        } else {
    ...
    private String calculateNewRigitz(int k, int i, int j){
        String r1;
        String r2;
        String r3;
        String r4;
        ...
        return union(r1, and(r2, star(r3), r4));
    }

    private String calculateDP(int k, int i, int j){
